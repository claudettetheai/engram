...
if [[ "$PROMPT" == "/clear" || "$PROMPT" == "/compact" || "$PROMPT" == "/clear "* || "$PROMPT" == "/compact "* ]]; then
  ...
  if [ -n "$SESSION_ID" ]; then
    ENGRAM_SESSION_ID="$SESSION_ID" ENGRAM_DB_PATH="$MEMORY_SCRIPTS" node -e '
      const { query, end } = require(process.env.ENGRAM_DB_PATH + "/lib/db");
      (async () => {
        try {
          await query(
            "UPDATE claude_memory.sessions SET status = '\''cleared'\'', ended_at = NOW() WHERE id = \$1",
            [process.env.ENGRAM_SESSION_ID]
          );
        } catch(e) { /* silent */ }
        await end();
      })();
